...
// Функция считывания начального состояния поля из файла. 
// Она принимает адрес первого элемента двумерного массива.
int input_field(int grid[ROWS][COLS]) {
    int success = 1; // Флаг. Если на какой-то из итераций считается некорректный символ из файла, допустим X,
                     // то флаг станет 0 и оба цикла остановятся.
    for (int i = 0; i < ROWS && success; i++) {
        for (int j = 0; j < COLS && success; j++) {
            int val = 0;
            // scanf возвращает количество успешно прочитанных аргументов
            // Если scanf не сможет прочитать что-то из файла, то вернет 0 и
            // условие будет ложным
            if (scanf("%d", &val) == 1) { 
                // Тернарный оператор. Ситнтаксис - Условие ? Выражение_1 : Выражение_2; 
                // Если условие верно (val не равен 0) - выполняется выражение 1,
                // если не верно (val равен 0) - выполняется выражение 2.
		// Эта функция также выполняет нормализацию. Любое
		// значение из файлика, которое scanf сможет прочитать, 
		// и которое не равно 0, превратится в 1 и сохранится в ячейку массива.
                grid[i][j] = (val != 0) ? 1 : 0; 
            } else {
		// если scanf не смог прочитать значение из файла,
		// то присваем статус ошибки и выходим из for-ов
                success = 0;
            }
        }
    }
